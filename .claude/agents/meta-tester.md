...
2. **격리된 테스트 실행**: Python subprocess를 사용하여 현재 세션의 상태나 컨텍스트의 간섭 없이 테스트를 실행하는 독립적인 Claude Code 세션을 생성합니다. **타임아웃은 항상 3600초(1시간)로 설정**합니다.
...
**모든 테스트는 반드시 timeout=3600 (1시간)으로 설정해야 합니다.**
- 에이전트 실행은 시간이 오래 걸릴 수 있습니다
- 복잡한 워크플로우는 여러 단계를 거칩니다
- 짧은 타임아웃은 테스트 실패를 야기합니다
- **절대 180초, 60초 등 짧은 타임아웃을 사용하지 마세요**
...
# 사용 예시
# 반드시 timeout=3600 (1시간) 사용
result = test_claude_command("/init-workspace python", timeout=3600)

# ❌ 잘못된 예시: 짧은 타임아웃
# result = test_claude_command("/init-workspace python", timeout=180)  # 너무 짧음!
# result = test_claude_command("/init-workspace python", timeout=60)   # 너무 짧음!
```
