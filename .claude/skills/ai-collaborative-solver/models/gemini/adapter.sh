...
# Gemini Model Adapter (Enhanced - Facilitator Compatible)
...
if [[ "$FACILITATOR_MODE" == "true" ]]; then
    ...
    # Just call Gemini once and return response immediately

    FULL_PROMPT="$PROBLEM

$([ "$ENABLE_SEARCH" = true ] && echo "**IMPORTANT:** Use Google Search to find latest information. Cite sources!" || echo "")

**Output Format:**
1. **Your Perspective**: Your unique viewpoint
2. **Key Points**: 3-5 main insights
3. **Evidence/Reasoning**: Support with facts
4. **Confidence Level**: 0-100%

Be concise but thorough."

    # Run Gemini with error handling
    if RESPONSE=$($GEMINI_CMD "$FULL_PROMPT" 2>&1); then
        echo "$RESPONSE"
    else
