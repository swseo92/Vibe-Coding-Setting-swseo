...
# Claude Model Adapter (Claude Code CLI - Facilitator Compatible)
...
# Detect facilitator mode: Check FACILITATOR_MODE env var (set by facilitator)
# Fallback: If DEBATE_CONTEXT is set, we're being called by facilitator
if [[ "${FACILITATOR_MODE:-}" == "true" ]]; then
    FACILITATOR_MODE=true
elif [[ -n "${DEBATE_CONTEXT:-}" ]]; then
    FACILITATOR_MODE=true
else
...
generate_uuid() {
    ...
    if command -v uuidgen &> /dev/null; then
        ...
        # Fallback: pseudo-random UUID format (works everywhere)
        # Note: python3 on Windows can be a broken Microsoft Store alias
...
CONVERSATION_FILE="$STATE_DIR/conversation.jsonl"
echo "" > "$CONVERSATION_FILE"  # Create/clear file
...
if [[ "$FACILITATOR_MODE" == "true" ]]; then
    ...
    # Just call Claude once and return response immediately

    RESPONSE=$(call_claude "$PROBLEM" "facilitator" "true")
