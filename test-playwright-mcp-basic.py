...
def test_playwright_mcp_basic():
    ...
    test_dir = Path(tempfile.mkdtemp(prefix="claude-test-playwright-basic-"))
    ...
    if platform.system() == "Windows":
        cmd = ["claude.cmd", "--print", "/mcp"]
    else:
        cmd = ["claude", "--print", "/mcp"]
    ...
    try:
        ...
        if "playwright-mcp" in result.stdout.lower():
            success_indicator = "✅ MCP server found in output"
        else:
            success_indicator = "❌ MCP server NOT found in output"
        ...
        print(f"Status: {'SUCCESS' if success and not error_indicators else 'FAILURE'}")
        ...
        return {
            "success": success and not error_indicators,
